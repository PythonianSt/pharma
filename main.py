...
async function saveEncounter(){{const first_name=document.getElementById("first_name").value.trim(),last_name=document.getElementById("last_name").value.trim(),hn=document.getElementById("hn").value.trim(),val=document.getElementById("icdSelect").value,out=document.getElementById("saveStatus");if(!first_name||!last_name){{out.innerHTML='<span class="danger">กรุณาระบุชื่อและนามสกุล</span>';return;}}if(!hn){{out.innerHTML='<span class="danger">กรุณาระบุ HN</span>';return;}}if(!selectedKeyword||!val){{out.innerHTML='<span class="danger">กรุณาเลือก ICD-10</span>';return;}}if(!rxItems.length){{out.innerHTML='<span class="danger">กรุณาเพิ่มยา</span>';return;}}if(!document.getElementById("confirm").checked){{out.innerHTML='<span class="danger">กรุณา Confirm</span>';return;}}const [icd10,diagnosis]=val.split("||");const payload={{first_name,last_name,hn,keyword:selectedKeyword,icd10,diagnosis,allergy:document.getElementById("allergy").value,current_med:document.getElementById("current_med").value,rx_items:rxItems}};const r=await fetch("/api/save-encounter",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify(payload)}});const j=await r.json();if(!r.ok){{out.innerHTML='<span class="danger">'+(j.error||"Save failed")+'</span>';return;}}out.innerHTML='<span class="ok">บันทึกแล้ว '+j.encounter_id+'</span>';rxItems=[];renderRx();document.getElementById("confirm").checked=false;}}
renderRx();
</script>
"""
